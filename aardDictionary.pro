...
SUBDIRS += aardDictionary \
           backend/AardDictionary
...
UBUNTU_TRANSLATION_DOMAIN="aarddictionary.danielbeck"
...
aptest.commands = QML2_IMPORT_PATH=$$OUT_PWD/backend bash $$PWD/aardDictionary/tests/autopilot/run
aptest.depends  = sub-aardDictionary sub-backend-App

unittest.target   = check
unittest.commands = /usr/bin/qmltestrunner -input $$PWD/backend/tests/unit -import $$OUT_PWD/backend
unittest.commands += && /usr/bin/qmltestrunner -input $$PWD/aardDictionary/tests/unit -import $$OUT_PWD/backend
unittest.depends  = sub-aardDictionary sub-backend-App
